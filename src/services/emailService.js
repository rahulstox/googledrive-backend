...
transporter.verify(function (error, success) {
  if (error) {
    ...
    console.log(
      "[EmailService] SMTP Connection Verified. Ready to send emails.",
    );
  }
});

const EMAIL_FROM =
  process.env.EMAIL_FROM || '"Krypton Drive" <no-reply@kryptondrive.com>';
...
export async function sendActivationEmail(email, firstName, activationLink) {
  ...
  try {
    ...
    console.log(
      `[EmailService] Email sent successfully to ${email}. MessageID: ${info.messageId}. Duration: ${Date.now() - start}ms`,
    );
    return info;
  } catch (error) {
    console.error(
      `[EmailService] Failed to send email to ${email}. Duration: ${Date.now() - start}ms. Error: ${error.message}`,
    );
    throw error;
  }
}
